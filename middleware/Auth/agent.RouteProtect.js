...
const AgentRouteProtect = async (req, res, next) => {

    try {

        const token = req.headers['authorization'];
        if (!token) {
            return res.status(401).json({ unauthorized: "unauthorized - No Token Provided 1" });
        ...
        try {
            ...
            if (!decoded) {
                return res.status(401).json({ unauthorized: "unauthorized - Ivaild Provided 2" });
            ...
            connection.query("SELECT * FROM agents WHERE ?", id, (err, results) => {
                if (err) {
...
export default AgentRouteProtect;
